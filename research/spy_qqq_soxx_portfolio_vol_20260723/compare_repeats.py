...
def locate(rep: str, name: str) -> Path:
    matches = sorted(ROOT.glob(f"**/*-{rep}/**/{name}")) + sorted(
        ROOT.glob(f"**/{rep}/**/{name}")
    )
...
def compare_frame(
    ...
    for column in exact:
        ...
        if not left.equals(right):
            ...
            raise RuntimeError(
                f"{name}: exact mismatch {column}: {difference.head().to_dict('records')}"
            )
    ...
    for column, tolerance in tolerances.items():
        ...
        if maximum > tolerance:
            raise RuntimeError(
                f"{name}: {column} max difference {maximum} > {tolerance}"
            )
...
manifest_exact = [
    ...
    "base_engine_source_sha256",
    "selection_policy",
    "raw_portfolio_vol_source_sha256",
    "patched_portfolio_vol_source_sha256",
    "deterministic_hotfix",
]
for key in manifest_exact:
    if manifest_a.get(key) != manifest_b.get(key):
        raise RuntimeError(
            f"manifest mismatch {key}: {manifest_a.get(key)} != {manifest_b.get(key)}"
        )
...
for symbol in sorted(inputs_a):
    for key in ["rows", "start", "end"]:
        if inputs_a[symbol][key] != inputs_b[symbol][key]:
            raise RuntimeError(
                f"input mismatch {symbol} {key}: "
                f"{inputs_a[symbol][key]} != {inputs_b[symbol][key]}"
            )
    difference = abs(
        float(inputs_a[symbol]["last_close"]) - float(inputs_b[symbol]["last_close"])
    )
    input_differences[symbol] = difference
    tolerance = 0.05 if symbol in {"SPY", "QQQ", "SOXX"} else 0.20
    if difference > tolerance:
        raise RuntimeError(
            f"input last-close mismatch {symbol}: {difference} > {tolerance}"
        )
...
identity_diff = compare_frame(
    ...
    tolerances={
        "production_target_vol": 0.0,
        "current_spy_weight": 0.005,
        "current_qqq_weight": 0.005,
        "current_soxx_weight": 0.005,
        "current_sso_upro_weight": 0.005,
        "current_qld_tqqq_weight": 0.005,
        "current_usd_soxl_weight": 0.005,
        "current_cash_weight": 0.005,
        "current_total_effective_exposure": 0.005,
        "cagr_delta": 0.0015,
...
family_diff = compare_frame(
    "family_summary.csv",
    keys=["family_key"],
    exact=["route", "family", "family_gate", "central_candidate"],
    tolerances={
        "strict_pass_count": 0.0,
        "central_cagr_delta": 0.0015,
        "central_cagr_vs_buy_hold": 0.0015,
...
result = {
    ...
    "policy": (
        "exact fixed-25%-target family/candidate identity; 22% and 28% robustness; "
        "actual 2x/3x product paths; matching input dates and frozen engine source"
    ),
}
